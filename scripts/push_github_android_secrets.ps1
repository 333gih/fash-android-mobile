...
# Usage: .\scripts\push_github_android_secrets.ps1 [-EnvFile secrets\android-release.env] [-Repo owner/repo]
param(
...
if (-not (Test-Path $EnvFile)) {
    Write-Error "Missing $EnvFile - copy secrets/android-release.env.example to secrets/android-release.env and fill in."
}
...
function Read-RepoFileOptional([string]$RelativePath) {
    ...
    $full = Join-Path $Root ($RelativePath -replace "/", "\")
    if (-not (Test-Path $full)) {
        Write-Host "skip file (not found): $RelativePath"
        return $null
    }
    return Get-Content -Raw -Encoding UTF8 $full
}
...
# Windows: trailing "\" on -Repo becomes part of the name → GitHub API 404 (%5C in URL).
$Repo = $Repo.Trim().TrimEnd('\', '/')
$repoArg = @()
if ($Repo -ne "") {
    if ($Repo -notmatch '^[\w.-]+/[\w.-]+$') {
        Write-Error "Invalid -Repo '$Repo'. Use owner/repo without trailing slash, e.g. fashandcurious14052026-dotcom/fash-android-mobile"
    }
    $repoArg = @("-R", $Repo)
}
...
if ([string]::IsNullOrWhiteSpace($keystoreB64) -and $envMap["ANDROID_UPLOAD_KEYSTORE_PATH"]) {
    ...
    if ($null -eq $keystoreB64) {
        Write-Host "skip ANDROID_UPLOAD_KEYSTORE_BASE64 (keystore file missing)"
    }
}
